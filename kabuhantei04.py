...
sheet3["G1"].value = "ﾗﾝｸ"
new_book.save(rf"C:\Users\manap\OneDrive\デスクトップ\hantei\kabuhantei04.xlsx")

conn = psycopg2.connect(host="localhost", database="manaponDB", user="postgres",password="manapon1219")
cursor = conn.cursor()

sql = 'select * from "kabuyosoku4" WHERE "scode0" > 0 ORDER BY "scode0" ASC'
cursor.execute(sql);
yosoku1 = cursor.fetchall()

sql = 'SELECT count(*) from "kabuyosoku4" WHERE "scode0" > 0 '
cursor.execute(sql);
...
for i in range(stpointer,endpointer):
  ...
  try:
    ...
    sql = 'update "kabuyosoku4" set filler01 = %s WHERE "scode0" = %s and "hiduke0" = %s '
